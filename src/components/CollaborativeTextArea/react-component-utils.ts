import { capitalize, CommandDecoratorMessageProps, CommandDecoratorShortcut, CommandDecoratorValue, getShortcutSymbols, I18nFormatter, includes, isArray, isEqual, isFunction, isString, ProsemirrorAttributes } from '@remirror/core';

/**
 * Get the value from the option passed into the command.
 */

export function getCommandOptionValue<Type>(value: CommandDecoratorValue<Type> | undefined, commandProps: CommandDecoratorMessageProps): Type | undefined {
...
export function getUiShortcutString(uiShortcut: CommandDecoratorShortcut, attrs: ProsemirrorAttributes): string {
...
export function getShortcutString(shortcut: string, options: ShortcutStringOptions): string {
    const { casing = 'title', namedAsSymbol = false, modifierAsSymbol = true, separator = ' ', t } = options;
    ...
    for (const sym of symbols) {
        ...
        if (sym.type === 'named') {
            const value = namedAsSymbol === true || (isArray(namedAsSymbol) && includes(namedAsSymbol, sym.key)) ? (sym.symbol ?? t(sym.i18n)) : t(sym.i18n);
